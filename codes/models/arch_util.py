...
def modified_softmax(x, dim=None):
    """
    ...
    Args:
        x (Tensor): input to softmax
        dim (int): the axis or axes along which the softmax should be computed.
        
    Returns:
        A tensor with the same shape as x.
    """
    m = torch.max(x.detach(), dim=dim, keepdim=True).values
...
class QKVAttentionLegacy(nn.Module):
    ...
    def forward(self, qkv, mask=None, qk_bias=0):
        ...
        weight = modified_softmax(weight.float(), dim=-1).type(weight.dtype)
