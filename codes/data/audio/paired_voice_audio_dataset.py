    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if text is None or len(text.strip()) == 0:
                raise ValueError
            if wav is None or wav.shape[-1] < (1.0 * self.sample_rate):
                # Ultra short clips are also useless (and can cause problems within some models).
                raise ValueError
            if wav is None or wav.shape[-1] > (20 * self.sample_rate):
                # Ultra long clips are also useless (and can cause problems within some models).
                raise ValueError
            if tseq.shape[0] > self.max_text_len:
                # Ultra long sentences are also useless (and can cause problems within some models).
                raise ValueError
