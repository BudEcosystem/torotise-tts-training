...
def load_audio(audiopath, sampling_rate):
    ...
    if audiopath[-5:] == '.flac':
        audio, lsr = audiofile.read(audiopath)
...
def load_similar_clips(path, sample_length, sample_rate, n=3, fallback_to_self=True):
    # We are getting the full path to audio files here print(path)
    ...
    candidates = glob.glob(f"{folder_name}/*.wav")
    # print(path,candidates[0], len(candidates))
    # else:
